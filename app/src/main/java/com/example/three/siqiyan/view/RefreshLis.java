...
public class RefreshLis extends ListView implements OnScrollListener {
	...
	public RefreshLis(Context context) {
		super(context);
		init();
	}

	public RefreshLis(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (currentState == REFRESHING) {// 判断当前状态是否为下拉刷新，是的话就不在刷新
				break;
			}

			int deltaY = (int) (ev.getY() - downY);// 获取滑动的长度

			int paddingTop = -headerViewHeight + deltaY;// 此时headview的padding
			if (paddingTop > -headerViewHeight
					&& getFirstVisiblePosition() == 0) {// 如果此时padding大于-headviewhead,并且当前第一个可见的条目是第一条则刷新
				...
				if (paddingTop >= 0 && currentState == PULL_REFRESH) {
					...
					refreshHeaderView();// 根据currentState来更新headerView
	...
	private void refreshHeaderView() {
		...
		case REFRESHING:
			...
			iv_arrow.setVisibility(View.INVISIBLE);// 箭头不可见
			pb_rotate.setVisibility(View.VISIBLE);// 进度条可见
